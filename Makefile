...
EXAMPLES = \
 examples/version-info \
 examples/expand-seed \
 examples/integers
...
BENCHDEPS = bench/lazer-bench.h lazer.h liblazer.a
BENCHLIBS = liblazer.a $(LIBS)

.PHONY: bench
bench: $(BENCH)
	cd bench && ./run-bench

bench/run-bench: bench/run-bench.c $(BENCHDEPS)
	$(CC) $(CPPFLAGS) $(CFLAGS) -I. -Ibench -o $@ $< $(BENCHLIBS)


EXAMPLEDEPS = lazer.h liblazer.a
EXAMPLELIBS = liblazer.a $(LIBS)

.PHONY: examples
examples: $(EXAMPLES)

examples/version-info: examples/version-info.c $(EXAMPLEDEPS)
	$(CC) $(CPPFLAGS) $(CFLAGS) -I. -o $@ $< $(EXAMPLELIBS)

examples/expand-seed: examples/expand-seed.c $(EXAMPLEDEPS)
	$(CC) $(CPPFLAGS) $(CFLAGS) -I. -o $@ $< $(EXAMPLELIBS)

examples/integers: examples/integers.c $(EXAMPLEDEPS)
	$(CC) $(CPPFLAGS) $(CFLAGS) -I. -o $@ $< $(EXAMPLELIBS)
